...
isTrain = c( rep(TRUE,2), rep(FALSE,1) )
```
...
gg = generateDiskData(
    ...
    maskIndex = 2,
    transformType = "rigid",
    noiseParameters = c(0, 0.1),
    sdSimulatedBiasField = 0.01,
    sdHistogramWarping = 0.01,
    sdAffine = 1, # lots of rotation
...
unetLM = patchMatchR::deepLandmarkRegressionWithHeatmaps( unet, activation = 'relu', theta=0 )
```
...
trtefns = read.csv( "numpy/LMtrainttestfiles.csv" )
...
```{r inference,echo=TRUE,eval=FALSE}
ptWeight = tf$cast( 0.05, mytype )
...
for (epoch in epoch:num_epochs ) {
  if ( (epoch %% round(mybs/batchsize) ) == 0 & epoch > 1  ) {
    ...
    mydf[epoch,'train_ptloss'] = as.numeric( losspt )
    mydf[epoch,'train_htloss'] = as.numeric( lossht )
    mydf[epoch,'ptWeight'] = as.numeric( ptWeight )
    if( epoch > 3 & epoch %% 10 == 0 ) {
      ...
      mydf[epoch,'test_loss'] = as.numeric( loss )
      mydf[epoch,'test_ptloss'] = as.numeric( losspt )
      mydf[epoch,'test_htloss'] = as.numeric( lossht )
      if ( mydf[epoch,'test_loss'] <= min(mydf[1:epoch,'test_loss'],na.rm=TRUE) ) {
